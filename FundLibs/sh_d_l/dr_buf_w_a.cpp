#include "dr_buf_w_a.h"

bool dr_buf_w_a::init(uint16_t w, uint16_t h) {
	this->w = w;
	this->h = h;
	buf = new col[w * h];
...
bool dr_buf_w_a::finit() { if (buf) delete[] buf; else return 0; buf = NULL; return 1; }

void dr_buf_w_a::dr_in_cont(HDC* dc, uint32_t scr_w, uint32_t scr_h) {
	if (buf) {
		if (scr_w == scr_h)
